...
### 5.2 목표 구조
```
pocket-llama-lab/                  # 루트(현 ollama-iphone-lab). 루트 폴더명 변경은 §14 결정
...
└── app/
    └── PocketLlama.xcodeproj + PocketLlama/
        ...
        ├── Models/                # ChatMessage, MessagesRequest/Response, SSEEvent, ClientError
        ├── Services/              # LLMChatClient(프로토콜) + AnthropicChatClient
        ├── Stores/                # AppSettingsStore (URL, 선택적 API Key)
        ├── ViewModels/            # ChatViewModel (+ ChatState 머신)
        ├── Views/                 # RootView, SettingsView, ChatView, ModelInfoView
        └── Assets.xcassets/
```
...
## 13. 완료 후 산출물 체크리스트
- [ ] §2 게이트 8줄 통과 기록
- [ ] 단일 git repo(`app/ server/ docs/ plans/`)
- [ ] **신규 생성**된 `PocketLlama` (실기기 실행)
- [ ] `server/serve.sh`(HOST 변형) + `test-anthropic.sh`
- [ ] 연결 테스트(`/health`) + 에러 분류
- [ ] 모델 표시(`/v1/models`) + fallback
- [ ] **멀티턴** 채팅(비스트림 검증 → 스트림 본선)
- [ ] 스트리밍(`SSEDecoder`) + 상태머신 + **Cancel**
- [ ] 설정·최근 대화 유지
- [ ] 보안 결정(무인증 경고 또는 api-key) 반영
...
- [ ] **보안** — 무인증 LAN 수용(권장 A) vs `--api-key`(옵션 B). *§2 게이트 6번 헤더 실측 후 최종 확정*
- [ ] 최소 iOS 배포타깃 — iOS 17 권장(신규 생성 시 설정)
- [ ] 루트 폴더명 `pocket-llama-lab` 변경 여부 / 계획서 파일명 `mvp-plan.md` 변경 시점
